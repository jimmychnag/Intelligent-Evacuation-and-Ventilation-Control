...
// I2C通訊 address+data
// address | A6 | A5 | A4 | A3 | A2 | A1 | A0 | R/W |     R/W = 0 寫，R/W = 1 讀 

#define SLAVE 0x4E //0x27 << 1

//I2C對LCD命令
// | P7 | P6 | P5 | P4 | P3 | P2 | P1 | P0 | 
// | D7 | D6 | D5 | D4 |BL:1| E  | RW | RS | 
...
void WriteInst4bits(unsigned char inst_4b)
{
		...
		send_8bits(SLAVE);  
		ack_receive();
		send_8bits(0x0C);         				//BL=1 EN=1 RW=0 RS=0
		ack_receive();
		send_8bits((inst_4b&0xF0)+0x0C);  //傳送D7-D4
		ack_receive();
...
{ 
		...
		send_8bits(0x0C);         			//BL=1 EN=1 RW=0 RS=0
		ack_receive();
		send_8bits((inst&0xF0)+0x0C);   //傳送D7-D4
		ack_receive();
		send_8bits(0x08);   						//EN=0 
		ack_receive();
		...
		send_8bits(0x0C);         			//BL=1 EN=1 RW=0 RS=0
		ack_receive();
		send_8bits((inst<<4)+0x0C);     //傳送D3-D0
		ack_receive();
		send_8bits(0x08);    						//EN=0 
		ack_receive();
		stop();
...
void WriteData(unsigned char data_)
{
		...
		send_8bits(0x0D);         			//BL=1 EN=1 RW=0 RS=1
		ack_receive();
		send_8bits((data_&0xF0)+0x0D);  //傳送D7-D4
		ack_receive();
		send_8bits(0x09);  
		ack_receive();
		...
		ack_receive();
		send_8bits((data_<<4)+0x0D);    //傳送D3-D0
		ack_receive();
		send_8bits(0x09);   						//EN=0 RW=0 RS=1
		ack_receive();
